#!/usr/bin/env bash
#*******************************************************************************
# File    : ~/.shell/hosts/death-star.sh
...
# DESCRIPTION:
# =============================================================================
# Setup colors and boolean for my workstation called death-star

# CHARACTER ENVIRONMENT SETUP
# =============================================================================
# If SHELL_APP variable if not already set. It can be not set if connected
# by ssh. Other if unkown, tty or shell app not listed as supporting unicode
# char
...
then
  ...
  # Show virtualenv char
  local VIRTUALENV_CHAR="P|"     # Default " "
  # Pwd, User and hostname char
...
# If shell is listed as supporting unicode char
else
  local PWD_CHAR=" "             # Default " "
  if [[ ${PROMPT_VERSION} -eq 1 ]]
  then
...
# Show keepass info
local SHOW_KEEPASS_INFO=true          # Default true
# Show tmux info
local SHOW_TMUX_ENV=true              # Default true
# Show kubernetes info
local SHOW_KUBE_ENV=true              # Default true
# Show openstack info
local SHOW_OPENSTACK_ENV=true         # Default true
# Show virtualenv info
local SHOW_VIRTUALENV_ENV=true        # Default true
# Show VCS info (now only git is supported)
local SHOW_VCS_ENV=true               # Default true
local VCS_COLORED=true                # Default false
local VCS_CHAR=${GIT_CHAR}            # Default ""
# Show untracked number of files in git info
local GIT_IGNORE_UNTRACKED=false      # Default "false"

# COLOR SETUP
# =============================================================================
# If SHELL_APP variable if not already set. It can be not set if connected
# by ssh. Other if unkown, tty or shell app not listed as supporting true colors
if [[ -n "${SHELL_APP}" ]] && \
  ([[ $(tput colors) -le 16 ]] || [[ ${SHELL_APP} == "tty" ]])
then
  # COLOR SETTINGS
  # ===========================================================================
  # To now if you terminal, program, etc. support true colors (i.e. 24 bits
  # colors) see https://gist.github.com/XVilka/8346728
  # Prompt colors if terminal support only 8/16 colors
  # Syntax from here https://misc.flogisoft.com/bash/tip_colors_and_formatting
  # By default, prompt default value a formatted for terminal that support 256
  # colors.
  if [[ ${PROMPT_VERSION} -eq 1 ]]
  then
    ...
    local HOST_FG="33"            # Yellow foreground
    ...
    local HOST_BG="43"            # Yellow background
    local HOST_FG="30"            # Black foreground
    local TMUX_BG="104"           # Light Blue background
...
elif [[ "${TRUE_COLOR_TERM[@]}" =~ ${SHELL_APP} ]]
then
  if [[ ${PROMPT_VERSION} -eq 1 ]]
  then
    # Prompt that support true colors
    ...
    local HOST_FG="38;2;175;135;0"          #rgb(175,135,0)
    ...
    local HOST_FG="38;2;0;0;0"              #rgb(0,0,0)
    local HOST_BG="48;2;175;135;0"          #rgb(175,135,0)
    ...
    local TMUX_BG="48;2;0;95;95"            #rgb(0,95,95)
...
then
  ...
  then
    ...
    local HOST_FG="38;5;136"          #af8700
    local KEEPASS_FG="38;5;23"        #005f5f
    local TMUX_FG="38;5;30"           #008787
...
# Equivalent hexadecime color for each code value for the 256 colors when term
# support it.value b
